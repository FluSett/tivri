...
func (r *PostgresRepository) Save(ctx context.Context, ld *Lead) error {
	query := "INSERT INTO intake_leads (company_name, project_scope, budget, contact_email, contact_phone, client_status, internal_status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id"
	err := r.pool.QueryRow(ctx, query, ld.CompanyName, ld.ProjectScope, ld.Budget, ld.ContactEmail, ld.ContactPhone, ld.ClientStatus, ld.InternalStatus).Scan(&ld.ID)
	if err != nil {
...
func (r *PostgresRepository) List(ctx context.Context) ([]Lead, error) {
	query := "SELECT id, company_name, project_scope, budget, contact_email, contact_phone, client_status, internal_status, created_at, updated_at FROM intake_leads ORDER BY id DESC"
	...
	for rows.Next() {
		var ld Lead
		err := rows.Scan(&ld.ID, &ld.CompanyName, &ld.ProjectScope, &ld.Budget, &ld.ContactEmail, &ld.ContactPhone, &ld.ClientStatus, &ld.InternalStatus, &ld.CreatedAt, &ld.UpdatedAt)
		if err != nil {
